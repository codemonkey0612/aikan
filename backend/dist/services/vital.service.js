"use strict";
var __createBinding = (this && this.__createBinding) || (Object.create ? (function(o, m, k, k2) {
    if (k2 === undefined) k2 = k;
    var desc = Object.getOwnPropertyDescriptor(m, k);
    if (!desc || ("get" in desc ? !m.__esModule : desc.writable || desc.configurable)) {
      desc = { enumerable: true, get: function() { return m[k]; } };
    }
    Object.defineProperty(o, k2, desc);
}) : (function(o, m, k, k2) {
    if (k2 === undefined) k2 = k;
    o[k2] = m[k];
}));
var __setModuleDefault = (this && this.__setModuleDefault) || (Object.create ? (function(o, v) {
    Object.defineProperty(o, "default", { enumerable: true, value: v });
}) : function(o, v) {
    o["default"] = v;
});
var __importStar = (this && this.__importStar) || (function () {
    var ownKeys = function(o) {
        ownKeys = Object.getOwnPropertyNames || function (o) {
            var ar = [];
            for (var k in o) if (Object.prototype.hasOwnProperty.call(o, k)) ar[ar.length] = k;
            return ar;
        };
        return ownKeys(o);
    };
    return function (mod) {
        if (mod && mod.__esModule) return mod;
        var result = {};
        if (mod != null) for (var k = ownKeys(mod), i = 0; i < k.length; i++) if (k[i] !== "default") __createBinding(result, mod, k[i]);
        __setModuleDefault(result, mod);
        return result;
    };
})();
Object.defineProperty(exports, "__esModule", { value: true });
exports.getVitalsPaginated = exports.deleteVital = exports.updateVital = exports.createVital = exports.getVitalById = exports.getAllVitals = void 0;
const VitalModel = __importStar(require("../models/vital.model"));
const VitalAlertService = __importStar(require("./vitalAlert.service"));
const getAllVitals = () => VitalModel.getAllVitals();
exports.getAllVitals = getAllVitals;
const getVitalById = (id) => VitalModel.getVitalById(id);
exports.getVitalById = getVitalById;
const createVital = async (data) => {
    const vital = await VitalModel.createVital(data);
    // バイタル記録作成後、アラートをチェック
    if (vital) {
        await VitalAlertService.checkVitalAlerts(vital.id);
    }
    return vital;
};
exports.createVital = createVital;
const updateVital = async (id, data) => {
    const vital = await VitalModel.updateVital(id, data);
    // バイタル記録更新後、アラートをチェック
    if (vital) {
        await VitalAlertService.checkVitalAlerts(vital.id);
    }
    return vital;
};
exports.updateVital = updateVital;
const deleteVital = (id) => VitalModel.deleteVital(id);
exports.deleteVital = deleteVital;
const getVitalsPaginated = (page, limit, sortBy, sortOrder, filters) => VitalModel.getVitalsPaginated(page, limit, sortBy, sortOrder, filters);
exports.getVitalsPaginated = getVitalsPaginated;
