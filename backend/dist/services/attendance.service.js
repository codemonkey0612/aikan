...
const checkIn = async (shift_id, user_id, lat, lng, pin) => {
     ...
     * チェックアウト（GPS位置情報付き）
     */
    export const checkOut = async (attendance_id, user_id, lat, lng, pin) => {
     ...
     * 出退勤ステータスの確認・更新
     */
    export const updateAttendanceStatus = async (attendance_id, status, type, pin) => {
        const attendance = await AttendanceModel.getAttendanceById(attendance_id);
        if (!attendance) {
     ...
     * PINコードを生成
     */
    export const generatePin = async (user_id, purpose, attendance_id) => {
        // 6桁のランダムPINを生成
    ...
    export const getAttendanceByShiftAndUser = AttendanceModel.getAttendanceByShiftAndUser;
};
